...
- **`fulfilment_status` needs an in-transit state, added with Sendcloud tracking (Phase 6), not before.** Both legs currently jump straight from label-printed to a manual close-out (`dispatched` → `completed`/`delivered`) with no "courier actually has it and it's moving" state in between — decided 20260811 to hold off rather than bolt on a manually-set status now. When Phase 6 lands, add the new value and have it set automatically off real Sendcloud tracking scans (both outbound-to-customer and return-to-us legs use the same status name, whichever leg is currently active), not a staff RPC. Until then, `mark_return_completed` and the dispatch/confirm_received flow are the only transitions.
